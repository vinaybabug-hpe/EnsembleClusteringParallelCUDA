...
#include "nvidia7_5/samples/advanced/cdpQuicksort.h"
...
//
////////////////////////////////////////////////////////////////////////////////
__global__ void qsort_warp(float *indata,
                           float *outdata,
                           unsigned int offset,
                           unsigned int len,
                           ...
                           unsigned int depth)
{
    ...
    float pivot = indata[offset + len/2];
    float data  = indata[offset + thread_id];
    ...
    if (lane_id == 0)
    {
        ...
        if (atomicAdd((unsigned int *) &atomicData->sorted_count, mycount) + mycount == len)
        {
            ...
            {
                if (source_is_indata)
                    cudaMemcpyAsync(indata+offset, outdata+offset, gt_len*sizeof(float), cudaMemcpyDeviceToDevice, lstream);
...
//
////////////////////////////////////////////////////////////////////////////////
float run_quicksort_cdp(float *gpudata, float *scratchdata, unsigned int count, cudaStream_t stream)
{
...
////////////////////////////////////////////////////////////////////////////////
// Modified version which takes data input
////////////////////////////////////////////////////////////////////////////////
int run_qsort(unsigned int size, float *data, int debug)
{
   ...
   // Create and set up our test
    float *gpudata, *scratchdata;
    checkCudaErrors(cudaMalloc((void **)&gpudata, size*sizeof(float)));
    checkCudaErrors(cudaMalloc((void **)&scratchdata, size*sizeof(float)));
...
//                if (i && !(i%32)) printf("\n        ");
    ...
    for (size=min; size<=max; size+=loop)
    {
        ...
        checkCudaErrors(cudaMemcpy(gpudata, data, size*sizeof(float), cudaMemcpyHostToDevice));
        ...
        checkCudaErrors(cudaMemcpy(data, gpudata, size*sizeof(float), cudaMemcpyDeviceToHost));
...
//                if (i && !(i%32)) printf("\n        ");
        ...
        if (check != size)
        {
        	if(verbose) printf("    cdpAdvancedQuicksort FAILED\n");
            exit(EXIT_FAILURE);
        }
        else if(verbose)
        ...
        if(verbose) printf("Sorted %u elems in %.3f ms (%.3f Melems/sec)\n", size, elapse, (float)size/(elapse*1000.0f));
        fflush(stdout);
    }
...
////////////////////////////////////////////////////////////////////////////////
// Default program which generates data random
////////////////////////////////////////////////////////////////////////////////
