...
//
////////////////////////////////////////////////////////////////////////////////
__device__ __forceinline__ int qcompare(float &val1, float &val2)
{
...
//
////////////////////////////////////////////////////////////////////////////////
static __device__ __forceinline__ void bitonicsort_kernel(float *indata, float *outdata, unsigned int offset, unsigned int len)
{
    __shared__ float sortbuf[1024];     // Max of 1024 elements - TODO: make this dynamic
    ...
    for (unsigned int k=2; k<=blockDim.x; k*=2) // Butterfly stride increments in powers of 2
    {
        for (unsigned int j=k>>1; j>0; j>>=1) // Strides also in powers of to, up to <k
        {
            ...
            float my_elem = sortbuf[threadIdx.x];
            float swap_elem = sortbuf[swap_idx];
...
//
////////////////////////////////////////////////////////////////////////////////
static __device__ __forceinline__ void big_bitonicsort_kernel(float *indata, float *outdata, float *backbuf, unsigned int offset, unsigned int len)
{
...
__global__ void bitonicsort(float *indata, float *outdata, unsigned int offset, unsigned int len)
{
    bitonicsort_kernel(indata, outdata, offset, len);
}

__global__ void big_bitonicsort(float *indata, float *outdata, float *backbuf, unsigned int offset, unsigned int len)
{
