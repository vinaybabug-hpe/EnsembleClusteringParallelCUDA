...
#include "rochester/gmm_gpu/gaussian.h"
#include "rochester/gmm_gpu/gaussian_cuda.h"
#include "rochester/gmm_gpu/invert_matrix.h"
...
static clusters_t* cluster(int original_num_clusters, int desired_num_clusters, int* final_num_clusters, int num_dimensions, int num_events, float* fcs_data_by_event) {
	...
	if (num_gpus < 1) {
		PRINT("ERROR: No CUDA capable GPUs detected.\n");
		return NULL;
	} else if (num_gpus == 1) {
		PRINT("Warning: Only 1 CUDA GPU detected. Running single GPU version would be more efficient.\n");
	} else {
	...
	clusters_t* tempclusters = (clusters_t*) malloc(sizeof(clusters_t) * num_gpus);
	...
	for (int g = 0; g < num_gpus; g++) {
		...
		tempclusters[g].N = (float*) malloc(sizeof(float) * original_num_clusters);
		tempclusters[g].pi = (float*) malloc(sizeof(float) * original_num_clusters);
		tempclusters[g].constant = (float*) malloc(sizeof(float) * original_num_clusters);
		tempclusters[g].avgvar = (float*) malloc(sizeof(float) * original_num_clusters);
		tempclusters[g].means = (float*) malloc(sizeof(float) * num_dimensions * original_num_clusters);
		tempclusters[g].R = (float*) malloc(sizeof(float) * num_dimensions * num_dimensions * original_num_clusters);
		tempclusters[g].Rinv = (float*) malloc(sizeof(float) * num_dimensions * num_dimensions * original_num_clusters);
		if (!tempclusters[g].means || !tempclusters[g].R || !tempclusters[g].Rinv) {
			printf("ERROR: Could not allocate memory for clusters.\n");
			return NULL;
		}
	...
	// Only need one copy of all the memberships
	tempclusters[0].memberships = (float*) malloc(sizeof(float) * num_events * original_num_clusters);
	if (!tempclusters[0].memberships) {
		printf("ERROR: Could not allocate memory for clusters.\n");
		return NULL;
	}
...
//	printf("CPU thread %d (of %d) using device %d: %s\n", tid, num_gpus, tid, prop.name);
	...
	CUDA_SAFE_CALL(cudaMalloc((void** ) &d_clusters, sizeof(clusters_t)));DEBUG("Finished allocating memory on device for clusters.\n");

	// Copy Cluster data to device
	CUDA_SAFE_CALL(cudaMemcpy(d_clusters, &temp_clusters, sizeof(clusters_t), cudaMemcpyHostToDevice));DEBUG("Finished copying cluster data to device.\n");
	...
	mem_size = num_dimensions * my_num_events * sizeof(float);
	...
	seed_clusters<<< 1, NUM_THREADS_MSTEP, 2*num_dimensions*sizeof(float) >>>( d_fcs_data_by_event, d_clusters, num_dimensions, original_num_clusters, my_num_events);

	// wait for child to complete
	cudaThreadSynchronize();
	if (cudaSuccess != (err=cudaGetLastError())) {

		DEBUG_PRINT("seed_clusters kernel error: %s", cudaGetErrorString(err));
		return NULL;
	}
...
//	float *matrixC;
//	CUDA_SAFE_CALL(cudaMalloc(matrixC, sizeof(float) * num_dimensions*num_dimensions));
	// Computes the R matrix inverses, and the gaussian constant
	constants_kernel<<<original_num_clusters, NUM_THREADS_MSTEP, num_dimensions*num_dimensions*sizeof(float)>>>(d_clusters,original_num_clusters,num_dimensions/*, matrixC*/);

//	constants(&temp_clusters, original_num_clusters, num_dimensions);
	constants_iterations++;
	cudaThreadSynchronize();
	/*	 wait for child to complete */
	if (cudaSuccess != (err=cudaGetLastError())) {

		DEBUG_PRINT("constants_kernel kernel error: %s", cudaGetErrorString(err));
		return NULL;
	}
...
//	seed_clusters(&clusters[0], fcs_data_by_event, original_num_clusters, num_dimensions, num_events);
	...
	for (int c = 0; c < original_num_clusters; c++) {
		...
		for (int d = 0; d < num_dimensions; d++) {
			for (int e = 0; e < num_dimensions; e++)
				DEBUG("%.2f ",clusters->R[c*num_dimensions*num_dimensions+d*num_dimensions+e]);DEBUG("\n\t");
		}DEBUG("R-inverse:\n\t");
		for (int d = 0; d < num_dimensions; d++) {
			for (int e = 0; e < num_dimensions; e++)
				DEBUG("%.2f ",clusters->Rinv[c*num_dimensions*num_dimensions+d*num_dimensions+e]);DEBUG("\n\t");
		}DEBUG("\n");DEBUG("\tAvgvar: %e\n",clusters->avgvar[c]);DEBUG("\tConstant: %e\n",clusters->constant[c]);
	...
	for (int num_clusters = original_num_clusters; num_clusters >= stop_number; num_clusters--) {
		...
		mem_size = (num_dimensions*num_dimensions*sizeof(float)) + (num_dimensions*sizeof(float));
		estep1<<<dim3(num_clusters,NUM_BLOCKS), NUM_THREADS_ESTEP, mem_size>>>(d_fcs_data_by_dimension,d_clusters,num_dimensions,my_num_events);
		...
		while (iters < MIN_ITERS || (fabs(change) > epsilon && iters < MAX_ITERS && change > 0)) {
			...
			CUDA_SAFE_CALL(cudaMemcpy(tempclusters[tid].N, temp_clusters.N, sizeof(float) * num_clusters, cudaMemcpyDeviceToHost));
			CUDA_SAFE_CALL(cudaMemcpy(tempclusters[tid].pi, temp_clusters.pi, sizeof(float) * num_clusters, cudaMemcpyDeviceToHost));
			CUDA_SAFE_CALL(cudaMemcpy(tempclusters[tid].constant, temp_clusters.constant, sizeof(float) * num_clusters, cudaMemcpyDeviceToHost));
			CUDA_SAFE_CALL(cudaMemcpy(tempclusters[tid].avgvar, temp_clusters.avgvar, sizeof(float) * num_clusters, cudaMemcpyDeviceToHost));
			CUDA_SAFE_CALL(cudaMemcpy(tempclusters[tid].means, temp_clusters.means, sizeof(float) * num_dimensions * num_clusters, cudaMemcpyDeviceToHost));
			CUDA_SAFE_CALL(cudaMemcpy(tempclusters[tid].R, temp_clusters.R, sizeof(float) * num_dimensions * num_dimensions * num_clusters, cudaMemcpyDeviceToHost));
			CUDA_SAFE_CALL(cudaMemcpy(tempclusters[tid].Rinv, temp_clusters.Rinv, sizeof(float) * num_dimensions * num_dimensions * num_clusters, cudaMemcpyDeviceToHost));
			...
			startTimer(timers.cpu);
			for (int c = 0; c < num_clusters; c++) {
				memcpy(&(tempclusters[0].memberships[c * num_events + tid * events_per_gpu]), &(temp_memberships[c * my_num_events]), sizeof(float) * my_num_events);
			}
			...
			mstep_covariance1<<<gridDim2, NUM_THREADS_MSTEP, num_dimensions*sizeof(float) >>>(d_fcs_data_by_dimension,d_clusters,num_dimensions,num_clusters,my_num_events);
//			int smemSize = (2*NUM_CLUSTERS_PER_BLOCK * sizeof(float)) + (NUM_THREADS_MSTEP*NUM_CLUSTERS_PER_BLOCK * sizeof(float));
//			mstep_covariance2<<<dim3((num_clusters+NUM_CLUSTERS_PER_BLOCK-1)/NUM_CLUSTERS_PER_BLOCK,num_dimensions*(num_dimensions+1)/2), NUM_THREADS_MSTEP>>>(d_fcs_data_by_dimension,d_clusters,num_dimensions,num_clusters,my_num_events);
			...
			startTimer(timers.constants);
			constants_kernel<<<num_clusters, NUM_THREADS_MSTEP,  num_dimensions*num_dimensions*sizeof(float)>>>(d_clusters,num_clusters,num_dimensions/*, matrixC*/);
			cudaThreadSynchronize();
		...
		memcpy(clusters[tid].N, tempclusters[tid].N, sizeof(float) * num_clusters);
		memcpy(clusters[tid].pi, tempclusters[tid].pi, sizeof(float) * num_clusters);
		memcpy(clusters[tid].constant, tempclusters[tid].constant, sizeof(float) * num_clusters);
		memcpy(clusters[tid].avgvar, tempclusters[tid].avgvar, sizeof(float) * num_clusters);
		memcpy(clusters[tid].means, tempclusters[tid].means, sizeof(float) * num_dimensions * num_clusters);
		memcpy(clusters[tid].R, tempclusters[tid].R, sizeof(float) * num_dimensions * num_dimensions * num_clusters);
		memcpy(clusters[tid].Rinv, tempclusters[tid].Rinv, sizeof(float) * num_dimensions * num_dimensions * num_clusters);
		memcpy(clusters[tid].memberships, tempclusters[tid].memberships, sizeof(float) * num_events * num_clusters);
...
//			tid, getTimerValue(timers.e_step) / 1000.0, regroup_iterations, (double) getTimerValue(timers.e_step) / (double) regroup_iterations / 1000.0, getTimerValue(timers.m_step) / 1000.0,
//			params_iterations, (double) getTimerValue(timers.m_step) / (double) params_iterations / 1000.0, getTimerValue(timers.constants) / 1000.0, constants_iterations,
//			(double) getTimerValue(timers.constants) / (double) constants_iterations / 1000.0, getTimerValue(timers.reduce) / 1000.0, reduce_iterations,
//			(double) getTimerValue(timers.reduce) / (double) reduce_iterations / 1000.0, getTimerValue(timers.memcpy) / 1000.0, getTimerValue(timers.cpu) / 1000.0);
	...
	// cleanup GPU memory
//	CUDA_SAFE_CALL(cudaFree(matrixC));
...
int cuda_gmm_main(int desired_num_clusters, float* fcs_data_by_event, int num_dimensions, int num_events, int *clusterIdx) {
	...
	if(!outf) {
		printf("ERROR: Unable to open file gmm for writing.\n");
		return -1;
	}
